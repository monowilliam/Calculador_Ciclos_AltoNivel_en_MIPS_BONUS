...
//Funcion limpiar pantalla que detecta la compilacion para windows y/o Linux
#ifdef _WIN32
...
//Creo una estructura para convertir enteros a string
namespace patch{
...
//Busca y guarda en dos vectores donde se inicializan los vectores y con que valor quedan.
...
//Dado dos string compara entre si para ver si son iguales en este caso es para buscar etiquetas con ":" igual a las sin punto
...
//Calcula el numero minimo entre dos valores
...
//Funcion principal que cuenta los ciclos en PIPELINED linea a linea
...
//Funcion opcional que calcula los ciclos en MONOCICLO linea a linea
...
//Crea una Matriz para el ciclo mas profundo que se va a desenrrollar
...
//busca dentro de la matriz principal del codigo el inicio (BRANCH) y final(J) del ciclo a desenrrollar
...
//Funcion que me determina cuandos ciclos existen en el codigo, si es un codigo de un ciclo simple o anidado
...
//Funcion que se encarga de imprimir la cantidad de desenrrollados por parte del usuario
...
//Funcion que imprime el menu del programa
void textoMenu(){
	cout<<endl ;
...
int main(void){
	...
	string version;
	cout <<endl<<" BIENVENIDO escriba por favor el nombre del codigo a probar ejemplo 'codigoProfe.asm' => ";
	version=obtenerString();
		do{
			matriz.clear();
			ciclo1.clear();
			contadorPipe=4;
			...
			operando1Ciclo="";
			operando2Ciclo="";
			abrirArchivo(version);   //Aqui va la ubicacion del archivo codigo
			calculadorDeCiclos();
			guardarRegistros();
			limpiar;
			textoMenu();
			...
			if( pl == "1"){
				...
				cout << endl; 
				cout << "Numero total de ciclos en PIPELINED son: "<<contadorPipe<<endl<<endl;
				system("pause");
			}
			else if( pl == "2"){
				limpiar;
				...
				cout << endl; 
				cout << "Numero total de ciclos en MONOCICLO son: "<<contadorMono<<endl<<endl;
				system("pause");
			}
			else if( pl == "3"){
				limpiar;
